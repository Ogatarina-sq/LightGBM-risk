...
def character():
    ...
    # 计算数据缺失比例，并将结果保存到csv中map
    # isnull.sum是计算每个特征的缺失值数目，即对于每列计算空值的数量，返回一个Series
    # shape计算数据集的行数，即数据集里所有的样本数 除上排，得每个特征的缺失值占总样本数的比例
    # sort_values是排序，false代表降序
    # {:.2%}格式化为保留两位小数的百分数字符串
    ...
    # 查找类型为非数值型标签
    # select_dtypes选取数据集df中的字符串类型特征object并返回DataFrame
    # apply 对DataFrame每一列应用函数 pd.series nunique 返回一个Series对象，表示该列的唯一值数量
    # axis=0 沿着列方向应用函数 返回Series对象
    ...
    for col in df:
        if df[col].dtype == 'object':
            #对某一列col进行缺失值填充，fillna是填充函数，取出众数数组的第一个元素。
            #inplace=True 直接在原始数据集本身进行缺失值填充，而不返回更改后的副本
